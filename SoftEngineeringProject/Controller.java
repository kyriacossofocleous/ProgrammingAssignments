	...
	public void sysAdminControl(){
		...
		if(input==1) {
			...
			if(input2==1) {
				...
				CandidateEmployee ce=new CandidateEmployee(name,title);
				boolean skillsEnough=false;
				while(!skillsEnough) {
					view.skillAdd();
					ce.addSkill(sc.nextLine());
	...
	public void createProposals() {
		view.welcomeProposalsAdministrator();
			...
			while (!addedEnough) { //allows for multiple proposals to be created
				if (!TeachingRequirements.getInstance().getListOfRequirements().isEmpty() //we need both requirements and candidates to be able to make proposals
